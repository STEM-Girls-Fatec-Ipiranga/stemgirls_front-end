...
export default function PostarVideos({
  ...
  // Lê vídeo como blob (preview) e base64 (salvar)
  const handleVideoChange = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Preview imediato (sem travar render)
    const blobUrl = URL.createObjectURL(file);
    setVideoPreview(blobUrl);
    setVideoFile(file);
  };

  // Lê thumbnail como base64
    ...
    <>
      <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
        <div className="bg-white p-6 rounded-2xl w-full max-w-2xl overflow-y-auto max-h-[90vh] border border-pink-200">
          ...
          {/* Upload vídeo */}
          <label className="block mb-2 font-semibold text-sm text-pink-600">
            Vídeo:
          </label>
          <input
            type="file"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 z-[999] flex items-center justify-center p-4">
          <div className="bg-black p-4 rounded-lg relative max-w-4xl w-full">
            ...
            <button
              ...
              className="absolute top-2 right-2 bg-white bg-opacity-80 hover:bg-opacity-100 text-black px-3 py-1 rounded-lg font-bold"
            >
              ✕
            </button>
          </div>
