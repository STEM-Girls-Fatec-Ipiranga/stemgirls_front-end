...
export default function Canais() {
  ...
  const canaisPadrao = [
    { id: 1, nome: "Tech World", descricao: "Dicas de tecnologia", banner: "/src/assets/img/FundoSGcolorido.jpg", fotoPerfil: "/src/assets/img/katherine-johnson.jpg", inscritos: 823, videos: [], owner: "system" },
    { id: 2, nome: "Mundo Gamer", descricao: "Gameplay e notícias", banner: "/src/assets/img/FundoSGcolorido.jpg", fotoPerfil: "/src/assets/img/grace-hopper.jpg", inscritos: 2310, videos: [], owner: "system" },
  ];

  // --- carregar canais e inscritos ---
  useEffect(() => {
    const all = JSON.parse(localStorage.getItem("canais_all"));
    setCanais(all && all.length ? all : canaisPadrao);
  ...
  // --- feed geral ---
  const feedVideos = canais.flatMap((c) =>
    (c.videos || []).map((v) => ({
      ...v,
      canalId: c.id,
      canalNome: c.nome,
      canalFoto: c.fotoPerfil,
      canalBanner: c.banner,
    }))
  ...
  return (
    <div className="flex h-screen bg-[#FFF6FF] text-black">
            ...
            {canaisFiltrados.length ? (
              <ul className="space-y-2 max-h-40 overflow-auto">
                {canaisFiltrados.map((c) => (
                  <li key={c.id} className="flex items-center gap-2 p-2 hover:bg-pink-50 rounded-lg cursor-pointer"
                    onClick={() => { setCanalSelecionado(c); setSearch(""); }}>
                    <img src={c.fotoPerfil} className="w-8 h-8 rounded-full object-cover" />
      ...
      <main className="flex-1 overflow-y-auto p-6">
        {canalSelecionado ? (
          <div>
            <button onClick={() => setCanalSelecionado(null)} className="text-pink-600 mb-4">← Voltar</button>
            <img src={canalSelecionado.banner} className="w-full h-48 rounded-xl object-cover mb-4" />
            <div className="flex items-center gap-4 mb-3">
              <img src={canalSelecionado.fotoPerfil} className="w-16 h-16 rounded-full object-cover" />
              <div>
          ...
          <section>
            <h2 className="text-2xl font-bold text-pink-600 mb-4">Feed de Vídeos</h2>
            {feedVideos.length ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {feedVideos.map((v, i) => (
                  <article key={i} className="bg-white rounded-xl shadow-md border border-pink-100">
                    {v.thumb ? (
                      <img src={v.thumb} alt="" className="w-full h-44 object-cover" />
                    ) : (
                      <video src={getPlayableUrl(v.url)} controls className="w-full h-44 object-cover" />
                    )}
                    <div className="p-4">
